...
import { getPortfolioPrompt } from '@/app/actions/get-portfolio-prompt'
...
export function AskPortfolioQuestionModal() {
  ...
  const handleCopyPrompt = async () => {
    ...
    try {
      ...
      const prompt = await getPortfolioPrompt()
          ...
          <div className="space-y-6">
              ...
              {copiedPrompt && copyStatus === 'error' && (
                <div className="mt-6 mb-6">
                  <label className="block text-sm font-semibold text-card-foreground mb-3">
                    Portfolio Prompt (Manual Copy Required)
                  </label>
                  <textarea
                    value={copiedPrompt}
                    readOnly
